    ...
    passport.deserializeUser(async (req, id, done)=>{ // id : admin
        try{
            ...
            if(req.body?.clientLogin){
                const selectClientQuery = `SELECT * FROM client WHERE user_id = '${req.body.user_id}'`;
                // const [clientInfo] = await pool.query(selectClientQuery);
                conn.query(selectClientQuery, (err, selectClientResult, fields) => {
                    if(err){
                        console.log(err);
                    }
                    else{
                        done(null, selectClientResult);
                    }
                })
            }else{
            }
